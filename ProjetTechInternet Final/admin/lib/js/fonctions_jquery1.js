$(document).ready(function () {
    ...
    $('#choix_produit').change(function () {
        ...
        let id_produit = $(this).val(); //on récupère la valeur du <select>, dans name
        console.log("name = " + name + " et id_produit = " + id_produit);
        let parametre = 'id_produit=' + id_produit;
        let retour = $.ajax({
            type: 'GET',
            data: parametre,
            datatype: 'json',
            url: 'admin/lib/php/ajax/ajaxRechercheProduit.php',
            success: function (data) {
                console.log(data);
                $('#description_produit').show();
                $('#image_produit').html("<img src='./admin/images/" + data[0].illustration + "' alt='produit'>");
                $('#detail_produit').html("Nom : " + data[0].libelle + "<br>Prix : " + data[0].prix);
            }
    ...
    $(this).blur(function () {
        ...
        if (valeur1 != valeur2) {
            ...
            var retour = $.ajax({
                ...
                data: parametre,
                dataType: "text",
                url: "./lib/php/ajax/ajaxEditproduit.php",
                success: function (data) {
...
$('#ajouter_produit').click(function(){
    ...
    $("#tableau_produits tbody").prepend(html);
    $('td[id]').blur(function(){
        let valeur = $(this).text();
        let champ = $(this).attr('id');
        let flag = true;
        $('#tableau_produits tr:gt(1)').each(function() { //"greater than" indice 1 (qui est la nouvelle ligne créée)
        ...
        if(flag == false){
            $('#nouveau_td').html("<span class='txtRed txtGras'>Produit déjà encodé</span>")
            ...
            let retour = $.ajax({
                ...
                data: parametre,
                datatype: 'json',
                url: './lib/php/ajax/ajaxInsertProduit.php',
                success: function () {
                    console.log("Ajouté");
                }
            })
...
$("td[id]").mouseover(function () {
    let id_produit = $(this).attr("id"); // valeur de l'id
    parametre = "id_produit=" + id_produit;
    let retour = $.ajax({
        type: 'GET',
        data: parametre,
        datatype: 'json',
        ...
        success: function (data) {
            console.log(data);
            $('#illustration').html("<img src='./images/" + data[0].illustration + "' alt='produit'>");
        }
    });
});
